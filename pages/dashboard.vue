<template>
  <a-row type="flex" justify="start">
    ...
    >
      ...
      <a-card class="mt-l">
        ...
        <a-card-meta>
          ...
          <template slot="description">
            <p>
              由于本系统还处于研发阶段，因此部分功能还未实现。您可以通过“开始审核”页面下方的“发起新投票”按钮获取一个新的句子以开始审核，点击句子下方的相关按钮提交您的审核意见。
            </p>
            <p>
              <strong
                >如果您发现您此时无投票权限，可以通过 QQ
                群组：734129102，申请权限。</strong
              >
              <br />
              请注意：系统会实时计算活跃审核员数目以生成基准票数；为了保护高阶审核员的权益，系统会自动
              <strong>暂停</strong> 超过 14
              天未操作过投票的审核员的投票权力。如果您因事，不幸被系统暂停了权限，可以再次向我们申请以开通权限。<strong
                >您的记录不会丢失。</strong
              >
            </p>
          </template>
      ...
      <a-card class="mt-xl avatar-card">
        <a-avatar
          class="card-top-avatar"
          :size="90"
          :src="getGravatarURLByEmail(user.email)"
        />
        <div class="mt-xs">
          <hr class="list" />
          <div class="list-child">标识：{{ user.id }}</div>
          <hr class="list" />
          <div class="list-child">名称：{{ user.name }}</div>
          <hr class="list" />
          <div class="list-child">邮箱：{{ user.email }}</div>
          <hr class="list" />
          <div class="list-child">身份：{{ user.role }}</div>
          <hr class="list" />
        ...
        <template slot="actions" class="ant-card-actions">
          ...
          <a-tooltip placement="bottom">
            <template slot="title"> 开始审核 </template>
            <a-icon type="solution" @click="viewPoll" />
          </a-tooltip>
          <a-tooltip placement="bottom">
            <template slot="title"> 结果与记录 </template>
...
export default class Dashboard extends Vue {
  public async asyncData({ app, store }: Context) {
  ...
  public getGravatarURLByEmail(email: string) {
    return this.getGravatarURLByHash(md5(email))
  }

  public getGravatarURLByHash(hash: string) {
    return 'https://fdn.geekzu.org/avatar/' + hash + '?d=mm&s=640'
  }

  public formatTime(input: string) {
