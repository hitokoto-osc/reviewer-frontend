      ...
      >
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'operation'">
            <a-button
              type="link"
              @click="showPollDetail(record['poll_id'] || 0)"
            >
              投票详情
            </a-button>
          </template>
...
.score-records {
  ...
  [data-doc-theme='light'] .ant-table-wrapper :deep(.table-striped) td {
    background-color: #fafafa;
  }

  [data-doc-theme='dark'] .ant-table-wrapper :deep(.table-striped) td {
