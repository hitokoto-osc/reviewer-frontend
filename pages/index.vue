<template>
  <a-row type="flex" justify="start" class="mt-5">
    <a-col
      :md="{ span: 8, offset: 8 }"
      :sm="{ span: 20, offset: 2 }"
      :xs="{ span: 22, offset: 1 }"
...
function hasErrors(fieldsError: { [index: string]: any }) {
...
export default class Index extends Vue {
  ...
  public form = this.$form.createForm(this, { name: 'authorization' })
  public isRequest = false
  public validatorText: string | undefined = ''
  ...
  public mounted() {
    ...
    if (this.getToken && this.getToken.length === 40) {
      ...
      const _this = this
      this.$axios
        .get(`https://poll.hitokoto.cn/v1/user/${this.getToken}`)
        .then((response) => {
  ...
  public tokenError() {
    ...
    console.log(this.form.isFieldTouched())
    // console.log(isFieldTouched('token') && getFieldError('token'))
    ...
    this.form.validateFields(async (err, values) => {
      if (!err) {
        ...
        const { data } = await this.$axios.get(
          'https://poll.hitokoto.cn/v1/user/' + token,
        )
        const { Code } = data
        if (Code !== 200) {
          // 验证失败
