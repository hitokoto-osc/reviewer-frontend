...
export default class Poll extends Vue {
  ...
  get pollSelect() {
    const select = JSON.parse(JSON.stringify(this.pollMark)) as PollMark[]
    select.map((v) => {
      v.value = v.id
      return v
    })
  ...
  formatPollType = formatPollType
  formatType = formatType

  async requestPoll(sentenceUUID: string, method: number, index: number) {
    ...
    if (this.markList[index] && this.markList[index].length > 0) {
      formData.append('marks', String(this.markList[index]))
    }
  ...
  async requestCancel(sentenceUUID: string, index: number) {
    if (this.requestPollLock[index]) {
      return
    }
    this.$set(this.requestPollLock, index, true)
  ...
  searchSentence(sentence: string) {
